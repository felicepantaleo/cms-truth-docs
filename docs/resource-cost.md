...
The MC-truth graph against the legacy frozen truth objects.
...
Compressed, the graph plus every association product is 137.4 kB/event smaller than the
legacy collections, a factor 1.22. The margin is much narrower than it was when this
document was first written: the graph grew when its detector scope was corrected (the
pruning and the hit index had been blind to the barrel, HCAL, the tracker and the muon
chambers, so the graph was small because it was wrong), and the association layer grew
from one working point to four plus the per-level denominators. The level flags and the
particle role added by the levels work cost 5 bytes per particle before compression.
...
!!! warning "Not re-measured"
    Measured on `CMSSW_20_1_X_2026-07-22-2300`. Section 1 has since been re-measured on
    `CMSSW_20_1_0_pre2`; treat the numbers below as the older release until they are redone.
